...
def process_success_condition(env: gym.Env, success_term: object | None, success_step_count: int) -> tuple[int, bool]:
    ...
    """
    target_pos = torch.tensor([0.2, 0.88, 0.0203], device=env.device)
    threshold = 0.5
    
    ee_pos = env.scene["robot"].data.body_pos_w[:, env.scene["robot"].find_bodies("openarm_left_ee_tcp")[0][0]]

    distance = torch.norm(ee_pos - target_pos, dim=-1)

    if distance < threshold:
        success_step_count += 1
        if success_step_count >= 5: 
            print(f"d：{distance.item():.4f}m")
    ...
    with contextlib.suppress(KeyboardInterrupt) and torch.inference_mode():
        while simulation_app.is_running():
            ...
            if running_recording_instance:
                ...
                # if teleop_interface.keyboard.buffer.get(_input.KeyboardInput.K):
                #     print("Kk")
                #     env.recorder_manager.record_pre_reset([0], force_export_or_skip=True)
                #     env.recorder_manager.set_success_to_episodes(
                #         [0], torch.tensor([[True]], dtype=torch.bool, device=env.device)
                #     )
                #     env.recorder_manager.export_episodes([0])
                #     print(f"path:{args_cli.dataset_file}")
                #     break
